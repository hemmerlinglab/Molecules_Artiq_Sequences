...
from bk9202_client import *


# every Experiment needs a build and a run function
class MOT_Current_Scan(EnvExperiment):
    def build(self):
        ...
        self.setattr_argument('setpoint_min',NumberValue(default=0,unit='A',scale=1,ndecimals=2,step=1))
        self.setattr_argument('setpoint_max',NumberValue(default=5.0,unit='A',scale=1,ndecimals=2,step=1))
    ...
    def run(self):
        ...
        self.bk.open()
        self.bk.switch_off()
        self.bk.set_voltage(10.0)
        self.bk.set_current(0.0)
        self.bk.switch_on()
        ...
        for n, nu in enumerate(self.scan_interval): 
            ...
            # change current of MOT power supply
            self.bk.set_current(nu)
