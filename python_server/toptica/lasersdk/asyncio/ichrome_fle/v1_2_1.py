# Generated from 'v1_2_1.xml' on 2025-07-21 10:00:25.694086
...
class Laser:
    ...
    async def gen_splitter_lookup(self, fiber: int) -> None:
        assert isinstance(fiber, int), f"expected type 'int' for parameter 'fiber', got '{type(fiber)}'"
        await self.__client.exec(self.__name + ':gen-splitter-lookup', fiber)

    async def clear_splitter_lookup(self, fiber: int) -> None:
        assert isinstance(fiber, int), f"expected type 'int' for parameter 'fiber', got '{type(fiber)}'"
        await self.__client.exec(self.__name + ':clear-splitter-lookup', fiber)

    async def load_splitter_lookup(self) -> None:
        await self.__client.exec(self.__name + ':load-splitter-lookup')

    async def disp_splitter_lookup(self) -> str:
        return await self.__client.exec(self.__name + ':disp-splitter-lookup', output_type=str)

    async def store_splitter_lookup(self) -> None:
        await self.__client.exec(self.__name + ':store-splitter-lookup')
...
class Beam:
    ...
    async def splitter_switch(self, fiber: int) -> int:
        assert isinstance(fiber, int), f"expected type 'int' for parameter 'fiber', got '{type(fiber)}'"
        return await self.__client.exec(self.__name + ':splitter-switch', fiber, return_type=int)

    async def splitter_rescue(self, array_size: int, step_size: int) -> None:
        ...
        await self.__client.exec(self.__name + ':splitter-rescue', array_size, step_size)

    async def optimize(self) -> int:
        return await self.__client.exec(self.__name + ':optimize', return_type=int)

    async def rescue(self) -> int:
        return await self.__client.exec(self.__name + ':rescue', return_type=int)
...
class AllLasers:
    ...
    async def switch_fiber(self, fiber: int) -> int:
        assert isinstance(fiber, int), f"expected type 'int' for parameter 'fiber', got '{type(fiber)}'"
        return await self.__client.exec(self.__name + ':switch-fiber', fiber, return_type=int)
...
class Switch:
    ...
    async def determine_ad_offset(self) -> None:
        await self.__client.exec(self.__name + ':determine-ad-offset')
...
class FLE:
    ...
    async def read_config_slave(self) -> bytes:
        return await self.__client.exec('read-config-slave', output_type=bytes)
