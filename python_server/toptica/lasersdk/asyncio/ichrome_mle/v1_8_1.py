# Generated from 'v1_8_1.xml' on 2025-07-21 10:00:25.922430
...
class Laser:
    ...
    async def reset_clip(self) -> None:
        await self.__client.exec(self.__name + ':reset-clip')

    async def store_config(self) -> None:
        await self.__client.exec(self.__name + ':store-config')

    async def gen_lookup(self) -> None:
        await self.__client.exec(self.__name + ':gen-lookup')

    async def disp_lookup(self) -> None:
        await self.__client.exec(self.__name + ':disp-lookup')

    async def store_lookup(self) -> None:
        await self.__client.exec(self.__name + ':store-lookup')

    async def load_lookup(self) -> None:
        await self.__client.exec(self.__name + ':load-lookup')

    async def clear_lookup(self) -> None:
        await self.__client.exec(self.__name + ':clear-lookup')

    async def check_lookup(self) -> float:
        return await self.__client.exec(self.__name + ':check-lookup', return_type=float)

    async def set_internal100(self) -> None:
        await self.__client.exec(self.__name + ':set-internal100')
    ...
    async def set_external100(self, power: float) -> None:
        ...
        await self.__client.exec(self.__name + ':set-external100', power)

    async def check_level(self) -> float:
        return await self.__client.exec(self.__name + ':check-level', return_type=float)

    async def recalibrate(self) -> None:
        await self.__client.exec(self.__name + ':recalibrate')

    async def store_calibration(self) -> None:
        await self.__client.exec(self.__name + ':store-calibration')

    async def wait(self) -> None:
        await self.__client.exec(self.__name + ':wait')

    async def restore_settings(self) -> None:
        await self.__client.exec(self.__name + ':restore-settings')
...
class Beam:
    ...
    async def shiftpolar(self, radial: int, tangential: int) -> None:
        ...
        await self.__client.exec(self.__name + ':shiftpolar', radial, tangential)
    ...
    async def shift(self, step_a: int, step_b: int) -> None:
        ...
        await self.__client.exec(self.__name + ':shift', step_a, step_b)

    async def moveto(self, xposition: float, yposition: float) -> None:
        ...
        await self.__client.exec(self.__name + ':moveto', xposition, yposition)

    async def gohome(self) -> None:
        await self.__client.exec(self.__name + ':gohome')

    async def sethome(self, use_current_settings: bool) -> None:
        ...
        await self.__client.exec(self.__name + ':sethome', use_current_settings)

    async def newhome(self) -> None:
        await self.__client.exec(self.__name + ':newhome')

    async def getpos(self) -> None:
        return await self.__client.exec(self.__name + ':getpos', return_type=None)

    async def newcenter(self) -> None:
        await self.__client.exec(self.__name + ':newcenter')

    async def checkcenter(self) -> None:
        return await self.__client.exec(self.__name + ':checkcenter', return_type=None)

    async def gocenter(self) -> None:
        await self.__client.exec(self.__name + ':gocenter')

    async def optimize(self) -> None:
        await self.__client.exec(self.__name + ':optimize')

    async def scan(self) -> None:
        await self.__client.exec(self.__name + ':scan')

    async def history(self) -> Tuple[str, int]:
        return await self.__client.exec(self.__name + ':history', output_type=str, return_type=int)

    async def goback(self) -> None:
        await self.__client.exec(self.__name + ':goback')

    async def gofwd(self) -> None:
        await self.__client.exec(self.__name + ':gofwd')
...
class Elliptec:
    ...
    async def findfreqs(self) -> str:
        return await self.__client.exec(self.__name + ':findfreqs', output_type=str)

    async def refinefreqs(self) -> str:
        return await self.__client.exec(self.__name + ':refinefreqs', output_type=str)
    ...
    async def move(self, time: int) -> None:
        ...
        await self.__client.exec(self.__name + ':move', time)

    async def fwd(self) -> None:
        await self.__client.exec(self.__name + ':fwd')

    async def back(self) -> None:
        await self.__client.exec(self.__name + ':back')
    ...
    async def run(self, frequency: int) -> int:
        ...
        return await self.__client.exec(self.__name + ':run', frequency, return_type=int)

    async def stop(self) -> None:
        await self.__client.exec(self.__name + ':stop')

    async def find_center(self) -> None:
        return await self.__client.exec(self.__name + ':find-center', return_type=None)
...
class Ibeam:
    ...
    async def cmd(self, cmd_string: str) -> None:
        assert isinstance(cmd_string, str), f"expected type 'str' for parameter 'cmd_string', got '{type(cmd_string)}'"
        await self.__client.exec(self.__name + ':cmd', cmd_string)

    async def reset_system(self) -> None:
        await self.__client.exec(self.__name + ':reset-system')
...
class Cam:
    ...
    async def grab(self) -> None:
        await self.__client.exec(self.__name + ':grab')

    async def max_level(self) -> int:
        return await self.__client.exec(self.__name + ':max-level', return_type=int)

    async def com(self) -> None:
        await self.__client.exec(self.__name + ':com')
...
class CamSetting:
    ...
    async def auto_adjust(self) -> int:
        return await self.__client.exec(self.__name + ':auto-adjust', return_type=int)
...
class Rect:
    ...
    async def get(self) -> Tuple[int, int, int, int]:
        return await self.__client.get(self.__name)

    async def set(self, left: int, top: int, width: int, height: int) -> None:
        ...
        await self.__client.set(self.__name, left, top, width, height)
...
class Script:
    ...
    async def txt(self) -> str:
        return await self.__client.exec(self.__name + ':txt', return_type=str)

    async def disp(self) -> str:
        return await self.__client.exec(self.__name + ':disp', output_type=str)
    ...
    async def store(self, newtxt: str) -> None:
        ...
        await self.__client.exec(self.__name + ':store', newtxt)

    async def read(self) -> bytes:
        return await self.__client.exec(self.__name + ':read', output_type=bytes)
    ...
    async def write(self, input_stream: bytes) -> None:
        ...
        await self.__client.exec(self.__name + ':write', input_stream=input_stream)

    async def exec(self) -> None:
        await self.__client.exec(self.__name + ':exec')
...
class MLE:
    ...
    async def hello(self) -> None:
        await self.__client.exec('hello')

    async def fw_update(self, input_stream: bytes) -> None:
        assert isinstance(input_stream, bytes), f"expected type 'bytes' for parameter 'input_stream', got '{type(input_stream)}'"
        await self.__client.exec('fw-update', input_stream=input_stream)

    async def save_counters(self) -> int:
        return await self.__client.exec('save-counters', return_type=int)

    async def debuglog(self) -> str:
        return await self.__client.exec('debuglog', output_type=str)

    async def servicelog(self) -> str:
        return await self.__client.exec('servicelog', output_type=str)

    async def errorlog(self) -> str:
        return await self.__client.exec('errorlog', output_type=str)

    async def summary(self) -> str:
        return await self.__client.exec('summary', output_type=str)
    ...
    async def restore_factory_settings(self) -> None:
        await self.__client.exec('restore-factory-settings')

    async def reboot_device(self) -> None:
        await self.__client.exec('reboot-device')
    ...
    async def change_password(self, passwd: str) -> None:
        ...
        await self.__client.exec('change-password', passwd)

    async def read_config(self) -> bytes:
        return await self.__client.exec('read-config', output_type=bytes)
